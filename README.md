...
## Custom bundle approach

> [!WARNING]
>
> **HIGHLY EXPERIMENTAL LOGIC**
>
> Please be advised that the following alternative initialization of the SDK is
> still under active development. Any methods, variables and functionality may
> change at any point.

Along side the default initialization of the SDK, an alternative way of
initializing the SDK is offered: the custom bundle approach.

The custom bundle offers a clean approach when it comes to how the SDK interfaces
with the OS as it does not make any use of swizzling or function overrides to
handle localization.

Instead of this, it relies into a custom generated bundle that is either
generated by the developer and then bundled together with the application bundle
and/or is generated when the localized strings are fetched over-the-air via
the `fetchTranslations` call.

There are some significant differences between the default SDK initialization
and the custom bundle approach:

* The default SDK initialization supports a custom locale provider. The custom
bundle approach **only supports the device or app specified locale by the user**.
* The default SDK initialization supports a complex caching mechanism that the
developer can override to their needs. The custom bundle approach **does not offer 
such option**.
...
**provides a more robust way of forward-compatibility support**.

### Initializing the SDK with the custom bundle approach

```swift
TXNative.initializeWithCustomBundle(sourceLocale: "en",
                                    appLocales: ["en", "el"],
                                    token: "<token>")
```

Developers can still call `TXNative.fetchTranslations()` to fetch translations
over the air.

When developers want to have a localized string translated via Transifex SDK,
they would need to provide the custom `.tfx` `Bundle` objects as the `bundle`
argument to their `String(localized: ...)`, `Text(...)` or
`NSLocalizedString(...)` calls:

```swift
let localizedString = String(localized: "You cannot pass.", bundle: .tfx)

Text("The wise speak only of what they know.", bundle: .tfx)

let nsLocalizedString = NSLocalizedString("Fly you fools", bundle: .tfx,
                                          comment: "Gandalf quote")
```
...
The `severity` property can be changed accordingly, so that the rule can be
stricter (e.g. `error`) or more relaxed (e.g. `warning`).
...
The first iteration of the custom bundle approach does not support custom
localization tables, which means that only strings found in the initial
...
* a Strings File (Legacy) (e.g. `Localizable.strings`, `InfoPlist.strings`, etc)
  where the Localization option has been enabled for every supported language of
  the app.
...
If you want a fast and easy workaround for the above issue, you can use the
`--dummy-strings` option on the CLI `pull` command that generates a
`tx-dummy.xcstrings` file you can copy to your Xcode project.

### Generating the custom bundle via the CLI tool
...
the `txstrings.json` file, so that the SDK can use the translations included in
that bundle before the over-the-air translations become available:

```console
txios-cli pull --token <TOKEN> --translated-locales en el --output ~/Desktop/ --export-type bundle
```
